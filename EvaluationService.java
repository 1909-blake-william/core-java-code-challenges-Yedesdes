...
public class EvaluationService {
	...
	public String cleanPhoneNumber(String string) {
		...
		if (filteredString.length() < P_NUMBER_LENGTH) {
			throw new IllegalArgumentException(
					"Phone Numbers must have at least " + P_NUMBER_LENGTH + " digits: " + string);
		}

		if (filteredString.length() > P_NUMBER_LENGTH + 1) {
			throw new IllegalArgumentException(
					"Phone Numbers must have at most " + (P_NUMBER_LENGTH + 1) + " digits: " + string);
		}
	...
	static class BinarySearch<T extends Comparable<T>> {
		...
		/**
		 * Provided with an object {@code t} to search for, searches the sortedList for
		 * the specified object using a binary search from lowIndex to highIndex. <br/>
		 * <br/>
		 * Note: This function assumes random access to the list elements, and a sorted
		 * list.
		 * 
		 ...
		 */
		private int indexOf(List<Comparable<T>> list, Comparable<T> searchFor, int lowIndex, int highIndex) {
		...
		while (parser.hasNext()) {
			...
			for (int i = 0; i < token.length(); i++) {
				...
				if (vowels.contains(token.charAt(i))
						&& !(i != 0 && token.charAt(i) == 'u' && token.charAt(i - 1) == 'q')) {
					vowelIndex = i;
					break;
				}
			}
	...
	/**
	 * Returns the set of prime numbers up to and including a specified long. The
	 * primes are in their natural ordering.
	 * 
	...
	static class AtbashCipher {
		...
		/**
		 * A private helper method to facilitate the encoding and decoding of atbash
		 * cipher.
		 * 
